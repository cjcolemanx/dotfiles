...
-- Allows folder expansion keybind to trigger editing if used on a non-direcory node
function M.expand_folder(node)
  -- local node = lib.get_node_at_cursor()
...
function M.change_root_to_global_cwd()
  ...
  print("Changed global cwd")
end

-- Add to module
M.git_add = git_add

function M.run_script_under_cursor(node)
  local api = require("nvim-tree.api")
  api.node.run.cmd()
end
